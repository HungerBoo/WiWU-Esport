...
- Build: Vite reads the six HTML entrypoints from `pages/` and generates a static `dist/` directory containing the six root-level `.html` routes and emitted assets.
...
| `pages/spielerprofil.html` | Individual WiWU player profile HTML entrypoint (`?player=<slug>`) | Mounts the shared app; Vite emits `dist/spielerprofil.html` |
| `pages/spielersuche.html` | Public League of Legends player search HTML entrypoint | Mounts the shared app; Vite emits `dist/spielersuche.html` |
...
| `public/images/Geschichte.png` | Local history image on the homepage | Referenced by `src/views/home.js` |
...
| `public/data/players.json` | Editable League and Smash roster source | Loaded by `src/views/game.js`; age is calculated from `birthDate`. Smash entries also carry a manually maintained `startggPlayerId`; their `stats` field is auto-generated by `scripts/update-startgg-stats.mjs` and should not be hand-edited |
...
| `scripts/update-riot-stats.mjs` | Node 20 script: refreshes League player `rank` and `lpHistory` from the Riot API, replacing same-day entries rather than duplicating them | Requires `RIOT_API_KEY` env var; run via `npm run update-riot-stats` |
| `scripts/generate-news.mjs` | Node 20 script: regenerates `public/data/news.json` from `players.json` via `src/content/news-generator.js` (rank-ups, deranks, tournament results) | Run via `npm run generate-news`, chained after stat updates in `npm run update-stats` |
| `scripts/fetch-smash-tournaments.mjs` | Manual/ad-hoc script to backfill recent start.gg tournament results for Smash players | Requires `STARTGG_API_TOKEN`; not part of the automated daily pipeline |
| `scripts/reconstruct-riot-history.mjs` | Manual/ad-hoc script to rebuild historical `lpHistory` for League players from the last 180 days of ranked matches (match-v5) | Requires `RIOT_API_KEY`; one-off maintenance tool, not part of the automated daily pipeline |
| `data/riot-cache.json` | Persistent cache of each League player's PUUID and LP history change-points | Read and written by `scripts/update-riot-stats.mjs` |
...
| `src/content/site-data.js` | Central site, social, game, and news metadata, including the Cloudflare Worker `riotProxyUrl` | Imported by page renderers; roster data lives in JSON |
| `src/content/news-generator.js` | Builds the news feed (rank-ups, deranks, tournament results, Prime League updates) from roster JSON | Used by `scripts/generate-news.mjs` and `src/views/home.js` (live client-side regeneration) |
| `src/content/players/<slug>.js` | Static per-player Steckbrief content (quotes, signature picks, links), one flat file per player | Aggregated by `src/content/players/index.js`, merged with live `players.json` data in `src/views/player.js` |
...
| `src/components/match-history.js` | Shared last-5 Ranked Solo/Duo match list with click-to-expand scoreboard | Used by `src/views/player.js` and `src/views/search.js` |
| `src/utils/roster.js` | Loads WiWU League PUUIDs from `players.json` so teammates get highlighted in match scoreboards | Used by `src/components/match-history.js` call sites |
...
| `src/utils/ddragon.js` | Shared Data Dragon CDN helpers (`getDdragonVersion`, `profileIconUrl`) | Used by `player.js`, `search.js` |
| `src/views/home.js` | Homepage content renderer with Prime League team overview | Called by `src/main.js` |
| `src/views/game.js` | Reusable game/team page renderer, roster JSON loader, and live SoloQ leaderboard | Called for League and Smash |
| `src/views/player.js` | WiWU player profile page: Steckbrief, live rank/LP graph, top champion masteries, Smash stats | Called for `spielerprofil.html` |
...
| `workers/riot-proxy.js` | Cloudflare Worker proxy: live Riot API rank lookups, optional summoner level/icon/top masteries (`?profile=1`), optional live/spectator game head-to-head view (`?live=1`), optional last N Ranked Solo/Duo matches (`?matches=5`), Data Dragon champion map, optional KV response caching | Deployed to Cloudflare Workers with `RIOT_API_KEY` secret and optional `SEARCH_CACHE` KV binding |
...
- Metadata includes a description and page title in the route shell; page content is rendered by `src/views/home.js`.
